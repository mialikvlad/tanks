...
import com.company.game.level.TileType;
...
public class Bullet {
    ...
    public enum BulletHeading {
        B_NORTH(20 * Player.SPRITE_SCALE, 6 * Player.SPRITE_SCALE + 4,
        ...
        B_SOUTH(21 * Player.SPRITE_SCALE, 6 * Player.SPRITE_SCALE + 4,
                Player.SPRITE_SCALE / 2, 1 * Player.SPRITE_SCALE / 2),
    ...
    private boolean canFly(float startX, float startY, float endX, float endY) {
        ...
        if (Integer.max(tileStartY, tileEndY) >= tileArray.length
            ...
            return false;
        } else if (isImpassableTileMetal(tileArray[tileStartY][tileStartX], tileArray[tileEndY][tileEndX])) {
            return false;
        }
        else
            return true;
    ...
    private boolean isImpassableTileMetal(Integer... tileNum) {
        for (int i = 0; i < tileNum.length; i++) {
            if (tileNum[i] == TileType.METAL.numeric()) {
                return true;
            }
        }
        return false;
